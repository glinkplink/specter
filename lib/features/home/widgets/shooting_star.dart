...
class _ShootingStarState extends State<ShootingStar>
  ...
  void initState() {
    ...
    _startTimer?.cancel();
    _startTimer = Timer(Duration(seconds: _delay.round()), () {
      if (mounted) {
        _startAnimation();
      }
    });
  ...
  void _randomizeTrajectory() {
    ...
    final seed =
        Random(widget.index + DateTime.now().millisecondsSinceEpoch ~/ 10000);
    ...
    if (isTopLeft) {
      ...
      _angle = 0.65 +
          seed.nextDouble() * 0.25; // 37-52 degrees (diagonal down-right)
    } else {
      // Top-right to bottom-left diagonal
      _startX = 0.8 + seed.nextDouble() * 0.2; // 80-100% from left
      _angle = 2.09 +
          seed.nextDouble() * 0.25; // 120-135 degrees (diagonal down-left)
    }
  ...
  void _startAnimation() {
    ...
    _controller.forward().then((_) {
      if (mounted) {
        ...
        _nextTimer?.cancel();
        _nextTimer = Timer(Duration(seconds: _delay.round()), () {
          if (mounted) {
            _startAnimation();
          }
        });
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final seed = Random(
            widget.index + DateTime.now().millisecondsSinceEpoch ~/ 10000);
        ...
        return AnimatedBuilder(
          ...
          builder: (context, child) {
            ...
            final currentX =
                startPosX + cos(_angle) * travelDistance * progress;
            final currentY =
                startPosY + sin(_angle) * travelDistance * progress;
...
class ShootingStarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gradient = LinearGradient(
      ...
      colors: [
        Colors.transparent,
        AppColors.lavenderWhite.withValues(alpha: 0.2),
        AppColors.lavenderWhite.withValues(alpha: 0.6),
        AppColors.lavenderWhite.withValues(alpha: 0.95),
        Colors.white,
      ],
